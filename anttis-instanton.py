...
def update_frame(frame_num):
    num_steps_per_frame = 10 # Run more steps for smaller DT
    ...
    # Dynamic clim based on current field, but bounded somewhat
    current_max = np.max(current_phi)
    current_min = np.min(current_phi)
    plot_vmax = max(current_max * 1.1, PARTICLE_AMPLITUDE * 0.5) # Don't let vmax drop too low
    plot_vmin = min(current_min * 0.8, -PARTICLE_AMPLITUDE * 0.2)
    if plot_vmax <= plot_vmin : plot_vmax = plot_vmin + 0.1 # Ensure vmax > vmin
    img.set_clim(vmin=plot_vmin, vmax=plot_vmax)


    positive_phi = np.maximum(0, current_phi) 
    if np.max(positive_phi) > 0.05 * PARTICLE_AMPLITUDE : # Lowered detection threshold
        try:
            threshold_value = 0.2 * np.max(positive_phi) 
            thresholded_phi = np.where(positive_phi > threshold_value, positive_phi, 0)
            if np.sum(thresholded_phi) > 1e-9: 
                 com_y, com_x = center_of_mass(thresholded_phi)
                 particle_marker.set_data([com_x * DX], [com_y * DX])
            else: 
                peak_y_idx, peak_x_idx = np.unravel_index(np.argmax(positive_phi), positive_phi.shape)
                particle_marker.set_data([peak_x_idx * DX], [peak_y_idx * DX])
        except Exception as e: 
            particle_marker.set_data([],[])
    else: 
        particle_marker.set_data([], [])

    ax.set_title(f"Particle-Field Sailing on Substrate (Φ)\nTime: {simulator.time:.2f}")
